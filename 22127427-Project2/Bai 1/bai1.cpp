...
BigInt modExp(BigInt base, BigInt exponent, const BigInt &modulus)
{
    BigInt result;
    result.sign = true;
...
int main(int argc, char *argv[])
{
    ...
    getline(inputFile, pHex, ' ');
    getline(inputFile, gHex);
    getline(inputFile, aHex);
    getline(inputFile, bHex);

    BigInt p = hexToInt(pHex);
    BigInt g = hexToInt(gHex);
    BigInt a = hexToInt(aHex);
    BigInt b = hexToInt(bHex);
    ...
    // Tính khóa chung
    BigInt keyAlice = modExp(B, a, p); // K = B^a % p
    BigInt keyBob = modExp(A, b, p);   // K = A^b % p

    try
    {
        // So sánh khóa chung bằng hàm compare
        if (compare(keyAlice, keyBob) == 0)
        {
            outputFile << BigIntToHex(keyAlice) << endl;
        }
        else
        {
            throw invalid_argument("Keys do not match!");
        }
    }
